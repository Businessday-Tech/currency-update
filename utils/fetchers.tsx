...
export async function getNgMarketData(): Promise<{
  ...
  bottom:ISymbolData[],
     ...
     const res1 = await fetch('https://doclib.ngxgroup.com/REST/api/mrkstat/topsymbols',{ next: {
      revalidate: 3600,
    },})
     const res2 = await fetch('https://doclib.ngxgroup.com/REST/api/mrkstat/bottomsymbols',{ next: {
      revalidate: 3600,
    },})
    // The return value is *not* serialized
    // You can return Date, Map, Set, etc.
...
export async function getMarketBlogData(): Promise<IPost[]> {
         ...
         const res = await fetch('https://businessday.ng/wp-json/wp/v2/posts?categories=1045',{cache: 'no-store'})
    // The return value is *not* serialized
    // You can return Date, Map, Set, etc.
    // console.log({res})
    if (!res.ok) {
