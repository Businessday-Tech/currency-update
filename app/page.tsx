...
import { getMarketBlogData, getParallelData, getPricesData } from "@/utils/fetchers";
...
export default async function Home() {
  ...
  const converter=Object.keys(data as IPriceData).reduce((acc,value)=>{
    if(value){acc[value as keyof IPriceData]=data?.[value as keyof typeof  data].reverse().find(value=>value[1]>27)?.[1] as number}
    return acc
},{} as {[K:string]:number})
