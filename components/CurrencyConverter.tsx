...
const CurrencyConverter = ({ data }: { data: { [K: string]: number } }) => {
  ...
  const handleValuesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCurrencies((x) => {
      if (name === "naira") {
        ...
        return {
          ...x,
          naira: numeral(nairaValue).format("0.00"),
          [name]: value,
        };
      }
    });
  ...
  const handleSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setCurrencies((x) => {
      let nairaValue =
        parseFloat(othersRef.current?.value ?? "0") * rates![e.target.value] ??
        "";
    ...
    <>
      <div className=" lg:px-6 mb-10">
        <div className="border border-slate-200 rounded-xl h-max mt-10 px-5 sm:px-12 py-10 pb-6">
          <h3 className=" text-2xl font-bold text-center mx-auto max-w-xl">
            Currency Converter(Instantly Calculate Exchange Rates and Values)
          </h3>
          <p className="font-lato text-[14px] text-center mt-3 mx-auto max-w-xl">
            Exchange rates can fluctuate frequently and can vary between
            financial institutions. Always verify rates with official sources
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between mt-10 relative">
            <div className="flex items-center justify-center border border-[#cdcdcd] w-full sm:w-0 basis-[100%] sm:basis-[45%] h-[92.98px] py-8 sm:py-0 px-2 lg:px-4 mb-5 sm:mb-0">
              <div className="flex flex-1 flex-col items-start justify-center gap-4">
                ...
                <input
                  name="naira"
                  type="number"
                  value={currencies.naira}
                  onChange={handleValuesChange}
                  placeholder="Amount"
                  className="placeholder:font-lato placeholder:text-[14px] font-bold border-0 pl-2 focus:outline-none focus:ring-0 focus:border-b focus:border-b-[#F91212] mt-1 w-full"
                />
              </div>
              <div className="flex flex-col  flex-1 items-end justify-between gap-4 text-[14px]">
            ...
            <div className="flex items-center justify-center border border-[#cdcdcd] w-full sm:w-0 basis-[100%] sm:basis-[45%] h-[92.98px] py-8 sm:py-0 px-4 mb-1 sm:mb-0">
              <div className="flex flex-col items-start justify-center flex-1 gap-4 w-full">
                <label htmlFor="Amount" className="font-lato text-[14px]">
                  Amount
                </label>
                <input
                  ...
                  type="number"
                  onChange={handleValuesChange}
                  value={currencies.others}
                  placeholder="Amount"
                  className="placeholder:font-lato placeholder:text-[14px] font-bold border-0 pl-0 focus:outline-none focus:ring-0 focus:border-b focus:border-b-[#F91212] mt-1 w-full"
                />
              </div>
              <div className="flex flex-col flex-1 items-end justify-between gap-4 font-lato text-[14px] w-full">
                <label htmlFor="Currency" className="font-lato text-[14px]">
                  Currency
                </label>
                <div className=" flex">
                  <Image
                    src={`https://flagicons.lipis.dev/flags/4x3/${currencyNames[
                      selected as keyof typeof currencyNames
          ...
          <div className=" flex justify-center">
            <button
              className="bg-[#F91212] hover-bg-[#F91212]/80 transition-all duration-150 ease-linear text-white px-[100px] py-[10px] text-base rounded-md mt-8"
              onClick={() =>
                setCurrencies({
                  naira: "",
                  others: "",
      ...
      <Script id="my-script">{data.gon_response}</Script>
    </>
  );
};
